...
export const AddProduct = ({ businessId, slug }: { businessId: number; slug: string }) => {
  const [ref, modal, toggleModal] = useToggleState();

  const initValue = { title: "", description: "", media: [] };

  const { execute, loading } = usePost("POST", `/business/product/${slug}`);
  const { uploading, uploadFile, response: dataImages } = useUpload<UploadsTypes>();
  ...
  const handleClose = () => {
    if (product.title !== "" || product.description !== "" || (product.media && product.media?.length > 0)) {
  ...
  const handleSubmitForm = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (product.title === "" || product.description === "" || images.length === 0) {
      setErrors(true);
      return;
    }

    const body = { title: product?.title, description: product?.description, media: images, businessId };
    execute(`/products`, body);
  };
