...
const Content = ({ data, slug }: { data: BusinessSectorTypes[] | undefined; slug: string }) => {
  return (
    <menu className="grid grid-cols-1 gap-4 mt-4 sm:grid-cols-2 place-items-center xl:grid-cols-3">
      {!data?.length ? (
        <h3 className="w-full col-span-1 mt-8 text-lg font-semibold text-center sm:text-2xl md:text-3xl sm:col-span-2 xl:col-span-3 text-gray/50">The products is not found</h3>
      ) : (
        data?.map((item, index) => (
