...
const Content = ({ data, slug }: { data: BusinessSectorTypes[] | undefined; slug: string }) => {
  return (
    <menu className="grid grid-cols-1 gap-4 mt-4 sm:grid-cols-2 place-items-center xl:grid-cols-3">
      {!data?.length ? (
        <h3 className="w-full col-span-1 my-8 text-lg font-semibold text-center sm:text-2xl md:text-3xl sm:col-span-2 xl:col-span-3 text-gray/50">The products is not found</h3>
      ) : (
        data?.map((item, index) => (
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-between gap-4 px-2 pb-2 border-b-2 sm:items-end sm:flex-row">
        <h1 className="text-xl font-semibold sm:text-2xl md:text-3xl">Products from {formatTitleCase(slug)}</h1>
      ...
      <div className="flex flex-col items-center justify-between gap-4 my-4 sm:flex-row">
        ...
        <AddProduct businessId={business?.data.id || 0} slug={slug} />
      </div>
      {loading ? (
