...
export const Services = ({ data, slugBusiness, id }: { slugBusiness: string; data: BusinessSectorTypes[] | undefined; id: number }) => {
  ...
  const handleSubmitImage = async (slug: string) => {
    ...
    if (element) {
      await uploadFile(files!, `type=business&category=${slugBusiness}&business-type=service`);
    }
  };
  ...
  const handleSubmitForm = (slug: string) => {
    ...
    if (!element?.slug) {
      ...
      executeUpdate(`/services/${element?.slug}`, body);
      return;
    }
  };
  ...
  return (
    <div className="w-full px-4 pt-20 mx-auto sm:px-8">
      <h3 className="subheading mb-8">Add Services</h3>
