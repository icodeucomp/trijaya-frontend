...
export const DeleteProject = ({ slugProject, slug, title }: { slugProject: string; slug: string; title: string }) => {
  const [ref, modal, toggleModal] = useToggleState();

  const { execute, loading } = usePost("DELETE", `business/project/${slug}`);

  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    execute(`/projects/${slugProject}`, {});
  };
            ...
            {loading ? (
              ...
              <form onSubmit={handleSubmit} className="w-full space-y-4">
                <div className="space-y-2">
                  <h1 className="mb-4 text-sm font-semibold text-center sm:text-start text-primary-1 sm:text-lg">Delete Project</h1>
