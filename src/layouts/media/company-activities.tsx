  ...
  return (
    <Container className="py-10 space-y-8 md:space-y-16 sm:py-16 md:py-20">
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        <Img src={selected} alt="project activities picture" className="w-full overflow-hidden rounded-lg h-80 sm:h-96 md:h-80 lg:h-96" cover />
          ...
          {Array.from({ length: splitDataToSix }, (_, i) => (
            <div key={i} className="grid grid-cols-2 gap-2 p-1 sm:grid-cols-3">
              {companyActivities.slice(i * 6, (i + 1) * 6).map((img, j) => (
                ...
                >
                  <Img src={img} alt={`Image ${j + 1}`} className="w-full h-32 sm:h-24 lg:h-36" cover />
                </div>
              ))}
            </div>
          ))}
