    ...
    <footer className="bg-primary overflow-hidden">
      <Container className="flex justify-between gap-8 py-8 text-light">
        <Motion tag="div" initialX={-40} animateX={0} duration={0.4} className="flex-1 space-y-4">
          ...
          <h4 className="text-lg sm:text-xl md:text-2xl font-semibold">{t("description")}</h4>
        ...
        <Motion
          ...
          duration={0.4}
          delay={0.4}
          className="flex-1 hidden grid-cols-1 gap-8 mt-4 sm:grid justify-items-center lg:grid-cols-2 xl:grid-cols-3"
        >
          {footerLists.map((footer, i) => (
            <div key={i} className="space-y-2 min-w-44">
